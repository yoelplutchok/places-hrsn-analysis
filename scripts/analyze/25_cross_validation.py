...
def run_cross_validation():
    ...
    # Keep raw values; standardize inside each fold using train-only statistics
    # to avoid pre-split information leakage (no functional impact on OLS R²,
    ...
    for outcome in outcome_cols:
        ...
        for model_name, features in model_specs.items():
            ...
            for fold_idx, (train_idx, test_idx) in enumerate(kf.split(X_raw)):
                # Compute standardization statistics on training fold only
                ...
                X_std = X_train_raw.std(axis=0, ddof=1)
                X_std = np.where(X_std == 0, 1.0, X_std)
                y_mean = y_train_raw.mean()
                y_std = y_train_raw.std(ddof=1) or 1.0

                X_train = sm.add_constant((X_train_raw - X_mean) / X_std)
                X_test = sm.add_constant((X_test_raw - X_mean) / X_std)
