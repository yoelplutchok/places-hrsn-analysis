...
def run_factor_analysis():
    ...
    # Save best loadings (raw EFA — may be Heywood under extreme inter-item correlations)
    if best_loadings is not None:
        best_loadings.to_csv(PATHS["final"] / "efa_loadings_efa_raw.csv")
        logger.info(f"Raw EFA loadings ({best_n_factors}-factor) saved to efa_loadings_efa_raw.csv")

    # Canonical bounded loadings via PCA + Kaiser-normalized promax (for Table 2)
    # ML-EFA on these data yields a Heywood case (loading > 1.0, communality > 1.0)
    # because pairwise inter-item correlations reach 0.99. PCA loadings are bounded
    # by construction; Tucker's congruence between EFA and PCA factor patterns is > 0.99,
    # so the substantive structure is identical.
    if best_n_factors == 2:
        rng_dev = np.random.RandomState(42)
        idx_dev = rng_dev.permutation(len(X))[:30078]
        X_dev_canonical = X.iloc[idx_dev]
        X_dev_std = (X_dev_canonical - X_dev_canonical.mean()) / X_dev_canonical.std()
        pca_canonical = PCA(n_components=2)
        pca_canonical.fit(X_dev_std)
        L_can = pca_canonical.components_.T * np.sqrt(pca_canonical.explained_variance_)
        # Kaiser-normalized varimax then promax (power=3)
        h = np.sqrt(np.sum(L_can ** 2, axis=1))
        Ln = L_can / h[:, None]
        n_v, n_f = Ln.shape
        R_can = np.eye(n_f)
        d_prev = 0
        for _ in range(500):
            comp = Ln @ R_can
            u, s, vt = np.linalg.svd(
                Ln.T @ (comp ** 3 - (1.0 / n_v) * comp @ np.diag(np.sum(comp ** 2, axis=0)))
            )
            R_can = u @ vt
            d_new = np.sum(s)
            if d_new - d_prev < 1e-6:
                break
            d_prev = d_new
        L_var_can = (Ln @ R_can) * h[:, None]
        H_can = np.sign(L_var_can) * np.abs(L_var_can) ** 3
        T_can = np.linalg.lstsq(L_var_can, H_can, rcond=None)[0]
        diag_vec = np.sqrt(np.diag(np.linalg.inv(T_can.T @ T_can)))
        T_can = T_can @ np.diag(diag_vec)
        P_can = L_var_can @ T_can
        # Sign-flip and reorder so Factor_1 = Material Hardship, Factor_2 = Social Isolation
        mat_idx = [hrsn_cols.index(c) for c in ["foodstamp", "foodinsecu", "housinsecu", "shututility", "lacktrpt"] if c in hrsn_cols]
        soc_idx = [hrsn_cols.index(c) for c in ["loneliness", "emotionspt"] if c in hrsn_cols]
        if abs(P_can[soc_idx, 0]).sum() > abs(P_can[mat_idx, 0]).sum():
            P_can = P_can[:, [1, 0]]
        if P_can[mat_idx, 0].sum() < 0:
            P_can[:, 0] *= -1
        if P_can[soc_idx, 1].sum() < 0:
            P_can[:, 1] *= -1
        ...
        canonical_df.to_csv(PATHS["final"] / "efa_loadings_canonical.csv")
        canonical_df.to_csv(PATHS["final"] / "efa_loadings.csv")
        logger.info("Canonical PCA + Kaiser-promax loadings saved (Table 2 source)")
